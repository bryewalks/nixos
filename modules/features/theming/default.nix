...
# import-tree skips it), this aspect threads theme/themeBuilder/themeName into
# the home-manager evaluation and carries the stylix configuration.
{ den, inputs, lib, ... }:
...
let
  ...
  stylix-config =
    ...
    let
      ...
      dracula-papirus = import ../../../pkgs/dracula-papirus.nix {
        inherit pkgs;
        lib = pkgs.lib;
...
in
{
  ...
  den.aspects.theming.provides.to-users.homeManager = {
    imports = [
